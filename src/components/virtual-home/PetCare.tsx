...
import { Heart, Utensils, Gamepad2, Moon, Star } from "lucide-react";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export const PetCare = () => {
  const { pets, userPet, loading, adoptPet, feedPet, playWithPet, restPet } = useVirtualHome();
  ...
  // If user has a pet, show care interface
  if (userPet) {
    return (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {pets.map((pet) => {
          ...
          return (
            <Dialog
              key={pet.id}
              open={isDialogOpen && selectedPet?.id === pet.id}
              onOpenChange={(open) => {
