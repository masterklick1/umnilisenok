# Исправление вылета Android и настройка Google Maps
...
1. Устранить закрытие Android-приложения через несколько секунд на экране авторизации.
2. Заставить нативную карту в родительском кабинете (`LocationMap.tsx`) работать через собственный Google Maps API-ключ.

## Подтверждённый риск при старте

Сейчас `App.tsx` монтирует `DeferredChildDeviceServices` на любом Android-устройстве сразу после первого кадра через таймер `0 мс`, в том числе на маршруте `/auth`. Из-за этого загружается модуль нативных сервисов (геолокация, background-geolocation, мониторинг и App plugin), хотя пользователь ещё не вошёл. Внутри компонента основная работа выключена флагом `active`, но сам модуль и плагины уже инициализированы. Это нужно исключить. Точная причина системного crash без Android logcat пока не подтверждена, поэтому диагностика входит в первый этап.

## План работ

### 1. Исправление вылета на авторизации

1.1. Перенести загрузку `ChildDeviceServices` за строгий шлюз:
- не импортировать и не монтировать модуль на `/auth`, `/join` и публичных страницах;
- загружать его только после завершения авторизации, определения роли `child` и готовности Capacitor bridge;
- родительский аккаунт также не должен запускать детские гео/monitoring-плагины.

1.2. Увеличить безопасную задержку до готовности native bridge и не обращаться к плагинам во время первого рендера.

1.3. Убрать опасное автоматическое `recreate()` WebView при падении renderer-процесса: оно может создавать цикл перезапусков. Оставить корректное логирование причины и безопасное восстановление без повторного немедленного запуска проблемного сервиса.

1.4. Добавить понятные startup-метки в Android logcat для этапов `MainActivity`, WebView, auth и запуска детских сервисов, не записывая токены или персональные данные.

1.5. Проверить два сценария отдельно:
- чистая установка → экран авторизации остаётся открытым минимум 30 секунд;
- вход родителя → детские нативные сервисы не запускаются;
- вход ребёнка → сервисы запускаются только после готовности bridge.

### 2. Подготовка в Google Cloud Console
...
### 5. Добавление секрета в Codemagic
...
5.5. Проверить, что приложение больше не закрывается на экране авторизации. Если Android всё же завершает процесс, снять точную причину командой:
```text
adb logcat -c
adb logcat AndroidRuntime:E chromium:E Capacitor:E UmniLisenok:E *:S
```
После запуска команды воспроизвести вылет и сохранить строки с `FATAL EXCEPTION`.

### 7. Диагностика на случай проблем
